...
雷达默认由 OpenWorker 使用当前 DeepSeek 配置和只读 Web 搜索生成最多五个机会，也可以显式切换到 Codex。每个新机会必须包含具体产品、目标买家、定价方式、第一单步骤，以及至少一个带公开直达链接和进入方法的销售渠道。找不到可核验渠道的候选不会保存。雷达只保存研究结果，不会自动做外联、购买、发布或创建账户。
