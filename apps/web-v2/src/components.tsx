...
export function Status({ value }: { value: string }) {
  const tone = ["succeeded", "completed", "active", "healthy", "pass"].includes(value) ? "positive" : ["failed", "critical", "fail"].includes(value) ? "negative" : ["running", "queued", "pending", "warning"].includes(value) ? "accent" : "neutral";
  return <span className={`status status-${tone}`}>{statusLabel(value)}</span>;
}

export function statusLabel(value: string): string {
  const labels: Record<string, string> = { queued: "排队中", running: "运行中", waiting_input: "等待输入", waiting_approval: "等待审批", succeeded: "成功", partially_succeeded: "部分成功", failed: "失败", cancelled: "已取消", active: "已启用", paused: "已暂停", archived: "已归档", draft: "草稿", retired: "已停用", healthy: "正常", completed: "已完成", reused: "已复用", pending: "处理中", pass: "通过", warning: "提醒", fail: "失败" };
