...
import { ArrowClockwiseIcon, ArrowRightIcon, CheckCircleIcon, ChatCenteredTextIcon, ClockCountdownIcon, PulseIcon, ShieldWarningIcon } from "@phosphor-icons/react";
import type { Approval, AuditLog, MonthlyFinanceSummary, Run, Schedule } from "@personal-os/vnext-contracts";
...
export function TodayPage() {
  ...
  if (runs.isLoading || schedules.isLoading || finance.isLoading || approvals.isLoading) return <LoadingBlock />;
  if (runs.error || schedules.error || finance.error || approvals.error) return <ErrorBlock error={runs.error ?? schedules.error ?? finance.error ?? approvals.error} />;
  ...
  return <motion.div initial={reduce ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="page-stack">
    ...
    <section className="metric-band" aria-label="今日概览">
      ...
      <Metric icon={<ShieldWarningIcon />} label="需要处理" value={String(attention.length)} hint="输入、审批、验收或恢复" tone={attention.length ? "warn" : undefined} />
    ...
    {attention.length > 0 && <section className="approval-inbox panel"><div className="section-heading"><div><h2>需要你处理</h2><p>系统把等待、审批、验收和失败恢复集中到这里。</p></div><Link href="/runs">进入运行中心</Link></div><div>{attention.slice(0, 8).map((run) => { const approval = pendingApprovals.find((item) => item.runId === run.id); return <Link href={`/runs/${run.id}`} key={run.id}><span>{attentionIcon(run)}<strong>{approval?.summary ?? attentionLabel(run)}</strong></span><small>{run.executorType} · {formatDate(run.createdAt)}</small></Link>; })}</div></section>}
