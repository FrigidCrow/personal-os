...
import type { Approval, AuditLog, MonthlyFinanceSummary, Run, RunDeposition, Schedule } from "@personal-os/vnext-contracts";
...
export function TodayPage() {
  ...
  if (runs.isLoading || schedules.isLoading || finance.isLoading || approvals.isLoading || failedDepositions.isLoading) return <LoadingBlock />;
  if (runs.error || schedules.error || finance.error || approvals.error || failedDepositions.error) return <ErrorBlock error={runs.error ?? schedules.error ?? finance.error ?? approvals.error ?? failedDepositions.error} />;
  ...
  return <motion.div initial={reduce ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="page-stack">
    ...
    <section className="metric-band" aria-label="今日概览">
      ...
      <Metric icon={<ShieldWarningIcon />} label="需要处理" value={String(attention.length + depositions.length)} hint="输入、审批、验收、恢复或沉淀" tone={attention.length + depositions.length ? "warn" : undefined} />
    ...
    {attention.length + depositions.length > 0 && <section className="approval-inbox panel"><div className="section-heading"><div><h2>需要你处理</h2><p>等待、审批、验收、失败恢复和知识沉淀都集中在这里。</p></div><Link href="/runs">进入运行中心</Link></div><div>{depositions.slice(0, 8).map((item) => <Link href={`/runs/${item.runId}`} key={item.id}><span><BookOpenTextIcon /><strong>Obsidian 沉淀失败：{item.title}</strong></span><small>{item.errorMessage ?? "打开运行后重试"}</small></Link>)}{attention.slice(0, Math.max(0, 8 - depositions.length)).map((run) => { const approval = pendingApprovals.find((item) => item.runId === run.id); return <Link href={`/runs/${run.id}`} key={run.id}><span>{attentionIcon(run)}<strong>{approval?.summary ?? attentionLabel(run)}</strong></span><small>{run.executorType} · {formatDate(run.createdAt)}</small></Link>; })}</div></section>}
