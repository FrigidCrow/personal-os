...
export class SqliteVNextStore implements VNextStore {
  ...
  updateSchedule(schedule: Schedule): Schedule {
    const result = this.connection.prepare(`UPDATE schedules SET work_spec_id=@workSpecId,name=@name,cron_expression=@cronExpression,timezone=@timezone,enabled=@enabled,catch_up=@catchUp,next_run_at=@nextRunAt,last_run_at=@lastRunAt,updated_at=@updatedAt WHERE id=@id`).run({ ...schedule, enabled: int(schedule.enabled), catchUp: int(schedule.catchUp) });
